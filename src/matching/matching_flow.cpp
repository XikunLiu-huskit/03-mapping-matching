...
MatchingFlow::MatchingFlow(ros::NodeHandle& nh) {
    ...
    // c. gnss data origin dk add
    gnss_sub_raw_ptr_ = std::make_shared<GNSSSubscriber>(nh, "/kitti/oxts/gps/fix", 1000000);
...
bool MatchingFlow::ReadData() {
    ...
    //dk add
    //gnss_sub_raw_ptr_->ParseData(gnss_data_buff_);
    return true;
}
...
bool MatchingFlow::UpdateMatching() {
    if (!matching_ptr_->HasInited()) {
        ...
        matching_ptr_->SetInitPose(gnss_odometry_);
